...
# 2026-09-08 用户截图与录屏补充（优先于旧版观察）

# FocusFlight 选航班、选座与出票对照

依据：用户本轮四张原版截图和两张当前 Windows 截图。静态截图无法确认原版搜索范围、随机权重、动画时长及完整座位总数，这些不作原版事实。

| 页面 | 截图观察 | 当前差异 | 本轮实现 |
| --- | --- | --- | --- |
| 选择航班 | 地图占主要区域；顶部返回、随机、搜索；底部时间刻度、横向候选和单一操作 | 下方整块控制台包含标题、出发机场与多条分隔线，占据约半屏 | 去掉控制台外框；顶部轻工具栏，底部集中刻度与候选。保持真实候选算法、上限180、步进5，最低30分钟 |
| 客舱 | 宽大弧形机身从机头逐渐展开；双风挡、中央支柱；A/C与D/F夹着行号过道 | 小机头图标和带短圆角的座位容器 | 连续机头轮廓与宽风挡，客舱内部独立滚动；保留原有座位数量 |
| 专注类型 | 座位标签、问题标题、低饱和色图标胶囊覆盖在机舱上 | 纵向表单，蓝色统一选项 | 复用原有五种真实类型，紧凑彩色胶囊，宽窗口三列、窄窗口两列；原有模态键盘逻辑不变 |
| 出票 | 地图上叠加深色票据，大机场代码，四字段两列，撕线和条码 | 外层大面板与横向四字段使票据显得像表单 | 独立暗色票据，两列信息、清晰字号，保持二维码/条码内容与值机。初稿260ms入场已由下方录屏修订替换为3000ms连续打印；减少动态时直接完成 |
| 历史 | 我的集中个人飞行记录 | 顶部独立航迹按钮与我的重复 | 删除重复入口，保留我的、历史列表和详情逻辑 |

搜索范围：复用本地机场搜索；可选择当前合法候选，其他匹配结果明确标记为非当前候选。随机仅在当前候选中等概率选择。不扩大现有六个候选或变更距离模型。草稿、选座、出票均不得创建 activeFlight。

## 2026-09-08 录屏实测修订

参考文件 1cf709836785a71b06885aadca59107d.mp4：26.88 秒，590×1280，57 fps，完整解码1532帧。浏览器解码曾重复首帧，已弃用该输出，以下以本机FFmpeg H.264解码结果为准。整片四分之一秒总览和全部逐帧文件位于 work/preflight-v3/reference。时间边界按关键帧观测，误差约0.1秒；缓动公式为拟合，不是原版源码。

| 原版时间 / 关键画面 | 位置、尺寸与运动观察 | 当前截图差异 | 修改位置 | 验收 |
| --- | --- | --- | --- | --- |
| 0–0.9s 客舱 | 机头顶约屏高8%，机身约屏宽92%；风挡在14–22%，首排约54% | desktop seats.png 有结构，缺少连续退场 | app.ts / preflight-layout.css | 最终 Windows 候选已实测，见对应关键帧与录屏 |
| 0.9–2.2s 场景覆盖层 | 中部覆盖约85%宽、22%高；先缩放展开后静止，点击后收回 | focus.png 原纵向表单已改彩色胶囊 | preflight-layout.css | 最终 Windows 候选已实测，见对应关键帧与录屏 |
| 3.1–4.1s 客舱退场 | 向上加速，先机头离开，再全部22排通过，最后尾部退出；不是小飞机图标移动 | 当前直接换stage | app.ts，实际客舱元素动画 | 最终 Windows 候选已实测，见对应关键帧与录屏 |
| 4.1–4.4s 地图衔接 | 地图显现、航线构图调整；打印槽约在屏高92% | 当前票据瞬间淡入 | map.select复用，票据容器 | 最终 Windows 候选已实测，见对应关键帧与录屏 |
| 4.4–7.4s 出票 | 宽约88%，最终票据约41%屏高；顶边从92%匀速上移至约48%；机场码→字段→条码依次出槽，尺寸基本不变 | ticket.png 260ms translate28px，缺少打印过程 | app.ts控制交互，ticket-frame裁切与长行程CSS | 最终 Windows 候选已实测，见对应关键帧与录屏 |
| 7.4–8.9s 出票完成 | 值机按钮出现；纸张稳定，等待用户 | 按钮提前可用 | app.ts仅动画结束解锁 | 最终 Windows 候选已实测，见对应关键帧与录屏 |
| 9.0–9.5s 值机 | 纸张上移至屏高23%左右，背景变暗；票宽几乎不变 | 当前checkin另一个比例 | checkin布局统一、入场 | 最终 Windows 候选已实测，见对应关键帧与录屏 |
| 10.5–12.3s 撕线操作 | 接触点沿虚线从左向右；这段包含用户手指拖动，不是固定自动时长 | 原拖拽/键盘已有 | 保留输入与完成阈值 | 最终 Windows 候选已实测，见对应关键帧与录屏 |
| 13.4–15.3s 票根分离 | 纸身保留，票根逐渐旋转约−12°并下移约一个票根高度；完成后保留 | 原版24px/−2°过弱 | 票根表现规则 | 最终 Windows 候选已实测，见对应关键帧与录屏 |
| 15.3–17.5s 开始登机 | 飞行模式信息和白按钮出现在票根下方；纸身/票根仍在 | 当前新页面移除票据 | 复用checkin DOM，保留票据直到登机 | 最终 Windows 候选已实测，见对应关键帧与录屏 |
| 17.5–19.0s 登机准备 | 全屏地图与明亮遮罩衔接，图标/两行文案/出发按钮逐渐清晰 | 当前小准备卡 | 全屏ready表现，保留原地图与真实GO边界 | 最终 Windows 候选已实测，见对应关键帧与录屏 |
| 20.9–23.0s 起飞 | 点击后准备遮罩退出，地图旋转与缩放跟随飞机，白尾迹沿航线 | 现有真实飞行视图已承担 | 不改飞行RAF、模型和尾迹 | 最终 Windows 候选已实测，见对应关键帧与录屏 |
| 25.8–26.88s | iOS系统控制中心，属于系统UI | 无对应业务 | 不实现 | 不适用 |

参考图的卫星/蓝色地图和iOS系统能力不替换现有MapLibre地图，不伪造会员、设置或系统拦截功能。此次桌面节奏拟合：客舱1050ms加速退场、纸张3000ms匀速打印、票根900ms分离、登机准备1100ms。打印用固定出口裁切配合纸张长行程，不是短距离淡入。减少动态时直接进入稳定状态；返回/Esc取消表现动画并恢复草稿，动画不写入持久化状态。


## 最终验收结果

- npm run build 通过；npm test 37/37；git diff --check 通过。
- 最终 Windows WebView 内嵌 JS/CSS 与 dist 文件逐字节哈希一致；候选 exe SHA256：4fb283ed3bc22e1f55c28bf27e762f806dba7c6f00b3ec9a3ccb15d05176e431。
- 实测30/60/120分钟、合法随机、本地搜索、地图实际指针点击、88座与末排滚动、专注覆盖层、二维码解码、打印期间禁止值机、返回/Esc取消、重复点击、减少动态与打印中缩放。
- 实测指针拖拽与键盘撕票，分离票根完整保留；开始登机与起飞之前草稿不创建航班。最小化恢复时真实时间继续；关闭重开保持原航班与暂停时间，恢复不重复创建。
- 自动降落采用隔离测试档案的临近截止时间验证，真实执行降落与历史持久化一次；未声称等待完整30分钟或验证物理睡眠唤醒。测试档案与正式版隔离。
- 首页空/有数据、进行中显隐、世界成就、机场明细、3D漫游、趋势各周期、我的历史与返回在 Windows 验证通过；地图规划后padding恢复中心。
- 参考片26.88秒完整解码观察；Windows流程录像20.389秒，保留实际帧时间，用户等待区间较短，不能用总片长比较动画快慢。各阶段配对见 comparison-times.json。
- 原版精确缓动函数、机场累计时间是否表示停留时间无法确认。当前机场累计值明确为到达航班累计飞行时间。保留现有MapLibre地图、五种专注类型与飞行视觉，没有伪造会员或系统功能。
- 尚需人工验收视觉接近程度与桌面节奏；未commit、merge或替换正式版。
